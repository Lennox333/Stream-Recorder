...
def compare_length(videos, speedup, edit_list) -> bool:
    total = 0
    time_format = "%H\\:%M\\:%S"
    
    # Convert strings to datetime objects
    print(f"Comparing {speedup} and {videos}")
    for video_index, video in enumerate(videos):
        if edit_list:
            for item in edit_list:
                item_index = item.get('index', 0)
                
                if video_index != item_index : 
                    total += get_length(video)
                    continue

                start_time = item.get('start', '00\\:00\\:00')
                end_time = item.get('end', '00\\:00\\:00')
    ...
    if (speed - 120 <= total//10 <= speed + 120):    
        print("speed up ok!")
        return True
...
def construct_args(files: list, video_to_edit = []) -> tuple[list, list]:
    ...
    fileType = fileType.replace(fileType, '.mp4')
    ...
    return [input_cmd, files, video_to_edit] 
def main():
    ...
    for task in waitlist:
        ...
        video_to_edit = task[2]
        subprocess.run(input_cmd, shell=True, text=True)
        output_dir = input_cmd[-1]

        
        comp_result = compare_length(videos_dir,output_dir, video_to_edit)
